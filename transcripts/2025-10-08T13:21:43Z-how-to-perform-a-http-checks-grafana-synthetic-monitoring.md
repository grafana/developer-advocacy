# How To Perform A HTTP Checks | Grafana Synthetic Monitoring
...
In this video, Bakola, a senior developer advocate at Grafana, provides a comprehensive tutorial on setting up HTTP checks using Grafana Cloud Synthetic Monitoring. He explains that HTTP checks are essential for verifying the availability and performance of web applications by monitoring response times and status codes to preemptively catch issues before they affect users. Bakola introduces Grafana Cloud Synthetic Monitoring as a robust blackbox monitoring solution that can run various checks globally. He guides viewers through the process of creating an HTTP check using Grafana's demo app, Quick Pizza, detailing the steps for configuration, including selecting probe locations, setting alerting thresholds, and reviewing the results on a dashboard. The video emphasizes the importance of proactive monitoring for ensuring application reliability and user satisfaction, encouraging viewers to explore further documentation for more insights.

## Chapters

00:00:00 Introductions and overview of the session  
00:01:30 Explanation of HTTP checks and their purpose  
00:02:50 Introduction to Grafana Cloud Synthetic Monitoring  
00:04:00 Overview of different check types available in Grafana  
00:05:30 Steps to set up your first HTTP check  
00:06:15 Selecting check type and naming your job  
00:07:00 Specifying request options for the HTTP check  
00:08:20 Configuring uptime and SSL options  
00:09:45 Setting up alerting options and thresholds  
00:11:00 Reviewing results and dashboard insights from the HTTP check  

# Setting Up an HTTP Check in Grafana Cloud Synthetic Monitoring
...
Before we dive into HTTP checks, let's discuss what Grafana Cloud Synthetic Monitoring is. Grafana Cloud Synthetic Monitoring is a black-box monitoring solution that allows you to proactively monitor services and applications by running checks from public and private probes located globally. This enables you to easily assess the availability, performance, and correctness of these services and applications.
...
Now, let's return to the HTTP check and walk through how to set up your first HTTP check with Grafana Cloud Synthetic Monitoring. 

An HTTP check is one of the simplest ways to validate that your application is available and responding correctly. With Grafana Cloud Synthetic Monitoring, you can send a request to an HTTP or HTTPS endpoint to measure uptime, track response latencies, and capture errors. These checks can be used to validate:

- Return status codes
- Response headers using regular expressions
- Response bodies using regular expressions

For more complex validations, consider checking out multi-step HTTP checks and scripted checks. HTTP checks can run from over 20 managed locations around the world or from private probes within your own network. The results provide clear insights into your application's health and performance, and can trigger alerts to keep you informed when issues arise.

## Steps to Set Up Your First HTTP Check

For this tutorial, we are using Grafana's demo app, **Quick Pizza**. To get started, you'll need a Grafana Cloud account. 

1. **Navigate to the Main Menu:**
   - Once you're on the homepage, open the main menu and navigate to **Testing and Synthetics**.
   - Select **Synthetics** and then click **Create a Check**.

You'll see various options for checks. All four check types can perform HTTP tests, but here's how to decide which one to choose:
...
2. **Configure the Check:**
   - Provide a job name. We'll use the name **Quick Pizza: HTTP**.
   - Select a request type. We'll choose **HTTP**.
   - Select a request target. We'll choose **GET** and set our endpoint to **https://quickpizza**.

When specifying your request target, you can include additional options by selecting **Request Options**. Here, you can add options like:
...
**Note:** If you choose to add a request body, the body size is limited to 64 KB.

3. **Define Uptime:**
   - Click the **Define Uptime** button.
   - For **Valid Status Code**, leave this blank to default to any 200 status code.
   - For **Valid HTTP Version**, leave this blank as well.
   - In the **SSL Options**, select **Ignore SSL Certification**. This section is where you can check if an SSL is present or not.
   - Set the **Timeout** to 3 seconds. If you don't receive a response in 3 seconds, the check will fail.

4. **Add Labels:**
   - Click the **Labels** button. 
   - Create custom labels for additional information that can be helpful when alerted about a test. Each label uses key-value pairs. We'll choose **Quick Pizza: Test**.

5. **Execution Settings:**
   - Click the **Execution** button.
   - You can choose between **Public** and **Private Probe Locations**. Here, decide from which probe locations to send the request. We'll choose **Montreal, Jakarta,** and **Stockholm** for this example.
   - Set the **Test Frequency** to run every 1 minute. Notice the estimated executions expected to occur per month.

6. **Alerting:**
   - Click the **Alerting** button. Effective alerting is essential for synthetic monitoring. When metrics deviate from expected performance, alerts increase awareness and trigger necessary actions.
   - You have two alerting options: **Per Check Alerts** or **Legacy Alerts**. We recommend **Per Check Alerts**, which allow you to create alerts based on the number of check failures in a specific time window or if there is a TLS certificate failure or latency threshold. 
   - We'll select all three types of **Per Check Alerts**:
     - **Fail Check**: Choose three of the 15 executions as the threshold to alert if failures occur in the last 5 minutes.
     - **TLS Certificate**: Check alerts if the target certificate expires in less than a specified time.
     - **Latency**: Alert if latency exceeds 200 milliseconds over the last 5 minutes.
...
This dashboard provides metrics such as:
